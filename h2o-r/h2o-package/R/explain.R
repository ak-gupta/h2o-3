...
.create_leaderboard <- function(models_info, leaderboard_frame, top_n = 20) {
  ...
  leaderboard <- cbind(data.frame(model_id = .model_ids(models_info$model_ids), stringsAsFactors = FALSE),
                       leaderboard)
  # make sure metrics are numeric even if they are missing (e.g., AUC, AUCPR in multinomial)
  leaderboard[, !names(leaderboard) %in% c("algo", "model_id")] <- lapply(leaderboard[, !names(leaderboard) %in% c("algo", "model_id")], as.numeric)
  leaderboard <- leaderboard[, colSums(is.na(leaderboard)) < nrow(leaderboard)]
  leaderboard <- leaderboard[order(leaderboard[[2]]),]
  names(leaderboard) <- tolower(names(leaderboard))
  names(leaderboard) <- gsub("pr_auc", "aucpr", names(leaderboard))
